...
class TestSuiteManager {
  ...
  async runUnitTests() {
    ...
    try {
      const command = `pnpm exec jest --config='${JSON.stringify(jestConfig)}' --passWithNoTests`;
  ...
  async runIntegrationTests() {
    ...
    try {
      const command = `pnpm exec jest --testMatch="${this.config.testPatterns.integration}" --runInBand`;
      const result = execSync(command, { encoding: 'utf8', stdio: 'pipe' });
  ...
  async runE2ETests() {
    ...
    try {
      ...
      const command = `pnpm exec playwright test --config=playwright.config.js`;
      const result = execSync(command, { encoding: 'utf8', stdio: 'pipe' });
  ...
  async setupTestEnvironment() {
    ...
    try {
      ...
      execSync('pnpm db:migrate:test', { stdio: 'pipe' });
      
      // Seed test data
      execSync('pnpm db:seed:test', { stdio: 'pipe' });
