...
describe('initCommand — real file creation', () => {
  ...
  it('creates CLAUDE.md with Reporecall marker section', async () => {
    ...
    expect(content).toContain('Trust Contract')
    expect(content).toContain('auto-injecting')
    expect(content).toContain('EMPTY or STALE')
    expect(content).toContain('action=store|recall|list|forget|explain')
    expect(content).toContain('Hooks (automatic)')
