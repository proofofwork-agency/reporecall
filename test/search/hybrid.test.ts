...
describe("HybridSearch graph and sibling expansion (3E)", () => {
  ...
  it("prefers user-facing auth flow files over backend auth infrastructure for broad prompts", async () => {
    ...
    const chunks = [
      ...
      { id: "provider", filePath: "mcp-server/src/auth/provider.ts", name: "ProjectOAuthProvider", kind: "class_declaration", startLine: 1, endLine: 60, content: "export class ProjectOAuthProvider { async authorize() {} async verifyAccessToken() {} }", language: "typescript", indexedAt: now },
